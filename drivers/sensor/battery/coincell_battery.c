...
// This file is a one-to-one copy of zmk/app/module/drivers/sensor/battery/battery_voltage_divider.c
// with the only change being the percentage lookup table changed to that of coincell batteries (Ln 81, Col 27)
// and the coincell_common.h included for the lookup function (Ln 20).

#define DT_DRV_COMPAT splitkb_coincell_voltage_divider
...
static int bvd_sample_fetch(const struct device *dev, enum sensor_channel chan) {
    ...
    if (rc == 0) {
        ...
        uint8_t percent = coincell_mv_to_pct(millivolts);
